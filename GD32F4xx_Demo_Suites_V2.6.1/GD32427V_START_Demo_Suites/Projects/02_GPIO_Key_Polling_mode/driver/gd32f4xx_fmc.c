...
unsigned int flashloader(char argc,char ** argv)
{
    int cmd = 0;
    uint32_t  addr = NULL;
    uint8_t temp;
    ...
    case 0:
        ...
        break;
    case 1:
        FlashInit(0x00,0x00,0x00,0x00,0x00,NULL);
        break;
    case 2:
        addr = myatohex(&argv[2][2]);
        temp = atoi(argv[3]);
        memset(test_data,temp,100);
        FlashWrite((void *)addr,0,16,(const char *)test_data);
        break;
    case 3:
         addr = myatohex(&argv[2][2]);
         FlashErase((void *)addr,0xff);
         break;
    default:
        break;
    }
...
LTSH_FUNCTION_EXPORT(flashloader,"test flashloader api");